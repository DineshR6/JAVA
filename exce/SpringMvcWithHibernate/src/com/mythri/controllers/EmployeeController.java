...
public class EmployeeController {
	...
	public ModelAndView logout(HttpSession session) {
		...
		session.invalidate();// delete current session id and create new id
	...
	@RequestMapping("/getAllEmpsWithoutLogin")
	public ModelAndView getAllEmpsWithoutLogin(HttpSession session) {
		Integer id = (Integer) session.getAttribute("myId");
		List<Employee> emps = employeeService.getEmployeesWithoutLogin(id);
		return new ModelAndView("showEmps1", "emps", emps);
	}
//------------------------------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------------------------------------
	...
	public ModelAndView showpassword() {
		Employee employee = new Employee();
		return new ModelAndView("changePassword", "command", employee);
	}

	@RequestMapping(value = "/setPassword", method = RequestMethod.GET)
	public ModelAndView pswd(@RequestParam("psd1") String pass1, @RequestParam("psd2") String pass2,
			@RequestParam("psd3") String pass3, HttpSession session) {
		Integer id = (Integer) session.getAttribute("myId");
		...
		if (validUser != null) {
			if (pass2.equals(pass3)) {
				...
				if (pswd != 0) {
					return new ModelAndView("updatePassword", "msg", " Password Updated Successfully");
				}
				return new ModelAndView("changePassword", "msg", " Password Updated Failed");
			}
			return new ModelAndView("changePassword", "msg", " Passwords Mismatched...!! Enter the same passwords");
		}
		return new ModelAndView("changePassword", "msg", "Enter The Correct Password");
	}
